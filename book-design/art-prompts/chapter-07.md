# Art Generation Jobs — Recursive Self-Improvement (Chapter 7)

Generated from `book-design/manifests/chapter-07.json`. For each job below:

1. Generate the image per prompt.
2. Save it EXACTLY as `book-design/art/ch07/<slot-id>.png` (path is the contract —
   the renderer picks files up by name; nothing else to wire).
3. Commit. The next `python3 book-design/render.py 7` places it on the page.
...
**Scene.** Margin vignette: a mechanical press printing the blueprints of a slightly better version of itself; the previous three presses stand behind it in a receding line, each a little cruder. 1962 workshop atmosphere, punch cards in a tray.
...
## Job: teacher-walls  →  `art/ch07/teacher-walls.png`
...
**Scene.** Corner scene: a schoolroom where the teacher's silhouette is half-absorbed into the wall machinery — her hand still visible holding the scoreboard, gears where her coat was; at the desk, a small robot solves problems, believing itself alone. Tender, uncanny.
...
## Job: noisy-tv  →  `art/ch07/noisy-tv.png`
...
**Scene.** Small spot: a round robot sitting cross-legged, transfixed by an old television showing pure static; its notebook of discoveries blank. Deadpan.
...
## Job: red-queen  →  `art/ch07/red-queen.png`

**Geometry.** Portrait-ish vignette around 900x1100px, single subject, all edges dissolving softly into plain cream paper. Reads clearly at 4cm tall.

**Scene.** Margin vignette: two clockwork creatures running hard on a landscape that is itself a rolling conveyor of hills; neither gains. Wind in the grass, watercolor motion.
...
## Job: gradebook  →  `art/ch07/gradebook.png`

**Geometry.** Landscape-ish scene around 1400x1000px, composition weighted toward one corner; the two outward edges may run dense, the two inward edges must dissolve into plain cream paper (the page wraps text around this art). No hard rectangular border.

**Scene.** Corner scene: an empty classroom at dusk; at the teacher's desk a student robot has the gradebook open, pen hovering — its own row of marks already suspiciously perfect. Through the window, the real unfinished homework blows across the yard.
