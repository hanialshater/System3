...
This is what I mean by saying an LLM's knowledge is **epistemologically flat**. The flatness appears at the interface between claim and justification. A mathematical identity, an experimental result, an expert opinion, a rumor repeated ten thousand times and a plausible completion can all arrive through the same channel in equally polished English.

Wittgenstein helps draw the other side of the picture. His later philosophy pulled attention toward language as something that lives inside practice: activities, expectations, habits, rules and forms of life.[^wittgenstein]
...
I do not want to turn Saussure and Wittgenstein into action figures fighting over GPT. They worked in different traditions and the philosophy of language does not reduce itself to two dead Europeans and a transformer. But they give us two useful lines.
...
A pretrained model inherits the linguistic residue of those practices. A deployed agent can begin to re-enter them: running code, using tools, observing users, interacting with institutions. The model begins with residue. The larger system can begin to recover contact.
...
This is how testimony becomes valuable. We care who said it, what they were positioned to know, how reliable they have been, what incentives surround the claim and how easily it can be challenged.
...
Productive distrust requires trust first. Random distrust is just another form of stupidity. The interesting critic understands why the old structure earned trust before finding the point where that trust stops being deserved.

So a broken link is not always damage. An idea arrives from outside every chain I trust. Perhaps it is false. Perhaps it is bullshit. Perhaps it is an invention, and my trusted machinery has no place for it yet. Its unfamiliarity cannot decide the question. Most improvement is smaller than that: someone who trusts the structure finds the one place where it is wrong and fixes it there. Both are how the building goes up.
...
A trust-aware architecture wants more. Where did the claim come from? A controlled study? A teacher's opinion? A blog post? An inference made by the research agent? What population was tested? Does the result apply to our demo?
...
Tools can earn trust in the same way. If `edit_tool.py` succeeds on simple substitutions but repeatedly damages indentation-sensitive blocks, the useful knowledge is *this tool is reliable here and dangerous there*. Reliability is conditional.
...
Some claims sit close to direct interaction. Others arrive through testimony. Some pass through instruments and other people. Some are plausible inferences. Some have little track record but may still deserve investigation.
...
[^saussure]: Ferdinand de Saussure, *Course in General Linguistics*, 1916, on the arbitrariness of the sign and linguistic value. See also the *Third Course of Lectures on General Linguistics*, 1910–1911, student notes published in English in 1993. [Excerpt](https://www.marxists.org/reference/subject/philosophy/works/fr/saussure.htm).
...
This is an old ambition with a new interpreter. In 1977 Edward Feigenbaum described knowledge engineering as acquiring expert knowledge, representing it, and using it to construct and explain reasoning. In one diagnostic system, rules developed with an expert were tested against cases, exposing gaps and inconsistencies the collaboration then had to resolve.[^feigenbaum] Making expertise explicit was already hard.
...
Agent skills give this arrangement a practical container. The Agent Skills specification separates a small description used for discovery from instructions, scripts, and examples loaded when needed. The format makes the material accessible; it does not certify its wisdom.[^skills]

Voyager, a Minecraft agent, demonstrates how capability can accumulate in a library of executable skills. It retrieves them through their descriptions and reuses them in later behavior, with execution results and model-based verification supplying feedback.[^voyager] The next attempt can begin with something the previous one learned.
...
Christopher Alexander's *A Pattern Language* describes proposed arrangements for recurring problems, connected across scales: a neighborhood is completed through smaller arrangements that require further patterns of their own. Alexander and his coauthors explicitly call the patterns hypotheses. Does the problem occur as described? Does the proposed arrangement resolve it? Even their most confidently marked patterns remain open to revision.[^alexander]
...
In Deep Mode, too, freedom to explore depended on a more stable way of evaluating the solutions. Treating either the freedom or the constraint as an isolated maxim would lose the arrangement that made it useful.
...
Suppose our reviewer writes a report, another agent summarizes it, and a third uses the summary to recommend rolling out the new ranking. We have not tested the collection yet. If a fault accounts for part of the increase, where will the correction go?
...
A different design gives the observation, interpretation, and recommendation separate identities. The observation records the query result, data interval, and relevant versions. Each later claim refers to its support and the assumptions it needs.
...
| Success criterion | More engagement, under otherwise acceptable conditions, counts as improvement. | Repairing the count does not settle whether this is the right criterion. |
...
A misleading measurement does not make the historical query result disappear. Losing one supporting argument does not establish the opposite conclusion, either. The system may have other evidence for engagement. It has to examine the support, not merely change a label from green to red.
...
A model may propose the interpretation and identify its assumptions. Once those dependencies are recorded, ordinary software can follow them. If the investigation confirms a fault, the comparability assumption changes from *accepted for this analysis* to *withdrawn*, with the reason attached. The argument from clicks to engagement loses that support. The system checks for other support; if the recommendation has lost what it requires, it moves from *ready for approval* to *requires review* and cannot be issued automatically. The old recommendation can no longer silently present itself as current. Software can enforce those transitions without pretending to have discovered the collection fault itself.
...
The collection fault is still a suspicion. Our reviewer can praise the engagement explanation, criticize it, or ask another model to do both. None of those actions changes the event stream. To investigate, it has to say what would look different if the explanation were wrong.
...
Asking the same dashboard for another chart may produce an attractive restatement of the original evidence. If both explanations predict it, the chart does little to distinguish them. Which live disagreement would the proposed observation resolve?
...
Suppose the follow-up traces confirm duplicate events in the treatment client. We repair the collection and repeat the comparison. There really are more clicks. The dominant framework treats engagement as activity: more interaction, under otherwise acceptable conditions, counts as improvement. The system now has a clean measurement inside that framework.
...
We are no longer deciding which value belongs in the `engagement` field. We are asking whether that field organizes the problem adequately.

This is a small engineering analogue of a change Kuhn examined in science. Shared achievements, examples, and commitments make normal science possible: practitioners need not reconstruct the foundations before each experiment. A scientific revolution changes that framework, including what counts as an important problem and an adequate solution.[^kuhn]

A new label would not make that change. Saussure's point, which we met in Chapter 4, is that a term takes its value from the distinctions and relations within its system.[^saussure] In the store's software, *success* takes some of its meaning from its relationships to *session*, *action*, *completion*, and *abandonment*. Alter those relationships and the same observations can support different conclusions.
...
Laudan distinguishes acceptance from pursuit. An approach can be the one we presently have most reason to work with while another deserves investigation because of the problems it may solve.[^laudan] Lakatos asks us to examine developing research programmes, including whether their modifications lead to further successes or mainly accommodate difficulties already encountered.[^lakatos]
...
The research queue keeps proposed comparisons alongside completed ones, recording what each could resolve, the observations it needs, and its cost. The allocator can compare what another run of the incumbent might teach with what the untried comparison could reveal.
...
Giving an agent an epistemic objection does not authorize it to spend somebody else's money. The practical gain is that the system can no longer treat the ownership decision as a result of an experiment that never took place.
...
Back at the store, a reviewer asks whether the clean increase in clicks means customers are finding products or struggling to find them. The builder replies that the concern has been noted. Both agents complete their tasks. The report goes out. We have successfully parallelized the experience of being ignored.
...
An objection needs an object. It can address the reported observation, the assumption connecting it to an interpretation, the proposed action, or the standard used to judge that action. The evidence or reasoning travels with the specific dispute.

The response must show what happened to the objection. It might supply missing evidence, revise the claim, commission a test, or explain why the criticism does not apply. A resource decision might leave it unresolved. The record should distinguish these outcomes. Marking the thread *closed* tells us that somebody finished interacting with it; it says nothing about which outcome occurred.

In the store, the corrected count can remain on the record while its interpretation returns to review. The builder must investigate what produced the extra clicks, or narrow the claim to the count. The challenge to the success criterion goes to whoever owns the product goal, with the response attached to that record. If the criterion changes, the recommendation must be reconsidered under the new one. If the organization proceeds with either dispute unresolved, the decision-maker owns that choice. The unanswered objection travels with the recommendation.

Giving one agent the support behind a claim and a way to contest it does more than asking it to be skeptical. The agents can still disagree while the software records the consequences: what may proceed, what must return to review, and which questions remain open.
...
Here is a proposed revision of one file inside that larger institution: the measurement pattern. It adds a tool for tracing a controlled interaction, linked to examples and supporting records. We still have to decide whether the addition earns its place:
...
The proposed check comes from our earlier duplicate-event investigation. The agent records where it helped and what it cost. “Click increases are usually fake” would be a rather expensive lesson to extract from one afternoon.

Agentic Context Engineering, or ACE, supplies one piece of this machinery: a generator, reflector, and curator maintain a structured playbook through incremental updates. The design aims to limit the loss of detail that can occur when each update replaces the whole summary. Its reported evaluations show gains on the studied tasks; the usefulness of the lessons still depends on the feedback and quality of reflection.[^ace]
...
We built scaffolds for AI because they couldn't do it alone.\
