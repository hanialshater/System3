...
> **Addendum 4 (2026-08-30, LLM-writing re-evaluation after the voice de-patterning pass):** measured against the rubric's LLM-writing checklist, the manuscript now sits at low risk. Census: one-liner density 22–39% across essay chapters (was up to 63%); the "X is not Y. It is Z." pivot at ~0.6 per 1,000 words with only signature instances surviving; "This/That is where" at 14 book-wide; em-dashes 1.6 per 1,000 words (well under LLM-typical); AI-slop transitions effectively zero (5 hits, 4 of them ordinary "more importantly"). Cold-read control samples from never-edited sections (Ch. 5 middle, Ch. 12 "What Are Humans For?") read as authored prose. **Remaining tells, ranked:** (1) research-inventory texture in Chs. 6–8 ("Researcher X did A; system Y added B") — a content-structure issue only the deferred compressions and the author's first-person experiment can fix; (2) standalone bold aphorisms (50, unchanged) — mostly definitional/signature; ~8 could un-bold, but that is an emphasis-design decision interacting with the pull-quote layer; (3) every chapter still exits on a forward-pointing hook — judged a deliberate authored serialization device, not a defect, but a blind reader should confirm. **Caveat:** this re-evaluation partly grades this evaluator's own edits; the honest confirmation is a blind re-read by a judge with no memory of the manuscript's history (fresh GPT session on chapters 7, 9, 11, or a human cold reader). Human-writing-feel estimate: ~8.7/10 book-wide, from roughly 7 at session start; the floor chapters (7–8, ~7.5) are limited by inventory texture, not sentence rhythm.
