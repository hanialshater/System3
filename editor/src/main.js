...
function renderPanel(){
 ...
 if(tab==='text'){
  ...
  const paragraphs=(section()?.markdown||'').split(/\n\s*\n/).filter(p=>!/^\s*(#|>|\[\^|```)/.test(p)&&p.trim()).map(p=>p.replace(/[*_`]/g,'').trim());
  pane.innerHTML=`<div class="panel-heading"><span>PASSAGE-ANCHORED ARTWORK</span></div><div class="settings"><p class="hint">Click an illustration on the page to crop, transform, or adjust its colors. It follows its passage when the text moves.</p>${figures.map(f=>`<article class="figure-card"><img src="${esc(f.art)}" alt="${esc(f.title)}"><div class="figure-body"><strong>${esc(f.title)}</strong><button class="edit-art-button" data-edit-art="${esc(f.id)}">Edit on canvas ↗</button><label>After passage<select data-anchor="${esc(f.id)}"><option value="${esc(f.anchor)}">${esc(f.anchor.slice(0,95))}</option>${paragraphs.map(p=>`<option value="${esc(p.slice(0,110))}">${esc(p.slice(0,90))}…</option>`).join('')}</select></label><label>Layout<select data-style="${esc(f.id)}">${[['wide','Wide illustration'],['left','Text wraps on the right'],['right','Text wraps on the left'],['plate','Full illustration page'],['hidden','Hidden']].map(([v,n])=>`<option value="${v}" ${f.style===v?'selected':''}>${n}</option>`).join('')}</select></label><label>Height · ${f.height} mm<input data-height="${esc(f.id)}" type="range" min="30" max="110" step="5" value="${f.height}"></label><button class="text-button" data-remove="${esc(f.id)}">Remove illustration</button></div></article>`).join('')}${!figures.length?'<p class="empty">No illustrations attached to this section.</p>':''}<label>Add an illustration<select id="add-art"><option value="">Choose artwork…</option>${art.map((a,i)=>`<option value="${i}">${esc(a.title)}</option>`).join('')}</select></label>${layout.figures.some(f=>!sections.some(s=>s.id===f.section))?'<button id="reattach" class="secondary">Move detached artwork here</button>':''}</div>`;
  for(const field of ['anchor','style','height'])pane.querySelectorAll(`[data-${field}]`).forEach(el=>el.onchange=e=>{checkpoint();const f=layout.figures.find(f=>f.id===el.dataset[field]);f[field]=field==='height'?+e.target.value:e.target.value;renderPanel();changed();});
  pane.querySelectorAll('[data-edit-art]').forEach(el=>el.onclick=()=>editArtwork(el.dataset.editArt));
  pane.querySelectorAll('[data-remove]').forEach(el=>el.onclick=()=>{checkpoint();layout.figures=layout.figures.filter(f=>f.id!==el.dataset.remove);renderPanel();changed();});
  $('#add-art').onchange=e=>{if(e.target.value==='')return;checkpoint();const a=art[+e.target.value];layout.figures.push({id:crypto.randomUUID(),section:selected,anchor:(paragraphs[0]||'').slice(0,110),art:a.url,title:a.title,style:'wide',height:65});renderPanel();changed();};
  if($('#reattach'))$('#reattach').onclick=()=>{checkpoint();layout.figures.forEach(f=>{if(!sections.some(s=>s.id===f.section)){f.section=selected;f.anchor=(paragraphs[0]||'').slice(0,110);}});renderPanel();changed();};
 }
}
...
async function boot(){
 ...
 $('#push').onclick=()=>withBusy(async()=>{
  ...
  baseMarkdown=markdown;baseLayout=JSON.stringify(savedLayout,null,2)+'\n';readBranch=branch;saveLocal();$('#read-branch').value=branch;
